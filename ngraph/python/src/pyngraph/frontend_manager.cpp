...
public:
    ...
    ngraph::PartialShape getPartialShape(IPlace::Ptr place) const override {
        PYBIND11_OVERLOAD_NAME(ngraph::PartialShape, PyInputModel, "get_partial_shape", getPartialShape, place);
    }
};
...
void regclass_pyngraph_InputModel(py::module m)
{
    ...
    im.def("get_partial_shape", [](InputModelShared& self, std::shared_ptr<Place> place) {
        return self.inputModel.getPartialShape(*place);
    });
